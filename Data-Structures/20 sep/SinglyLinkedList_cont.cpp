...
public:
    ...
    void reverse();
    int count();
    Slist<T>& operator=(const Slist<T> &l2);
    void operator+(const Slist<T> &s2);
};
...
template<class T>
Slist<T>& Slist<T> :: operator=(const Slist<T> &li2) 
{
    node<T> *p;
    while(!isempty())
    {
...
int main()
{
    ...
    do
    {
        ...
        cout << "13) Assignment operator overloading" << endl;
        cout << "14) Exit" << endl;
        ...
        case 13:
            if(li.isempty())
            {
                cout<<"Oops! Linked List is Empty."<<endl;
            }
            else
            {
                li2 = li;
